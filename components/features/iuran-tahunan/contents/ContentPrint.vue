    ...
    <table class="min-w-full divide-y divide-black">
      ...
      <tbody class="bg-white">
        <tr class="text-caption">
          ...
          <td class="text-left px-2 py-1 capitalize">Pembayaran ansuran iuran tahunan</td>
          <td class="text-right px-2 py-1">Rp {{currencyFormat(ansuran)}}</td>
        </tr>
      </tbody>
    </table>
    <div class="border-t border-black pt-2 flex justify-end">
      <div class="flex flex-col">
        <div class="flex flex-row text-caption pr-2 justify-end">
          <p class="font-medium mr-4">IURAN Periode Ini: </p>
          <p class="font-medium">Rp. {{currencyFormat(dataIuran.iuran)}}</p>
        </div>
        <div v-if="dataIuran.potongan" class="border-b border-black flex flex-row text-caption pr-2 justify-end">
          <p class="font-medium mr-4">Beasiswa / Potongan: </p>
          <p class="font-medium">Rp. {{currencyFormat(dataIuran.potongan)}}</p>
        </div>
        <div class="flex flex-row text-caption pr-2 justify-end">
          <p class="font-medium mr-4">Sudah Dibayar: </p>
          <p class="font-medium">Rp. {{currencyFormat(dataIuran.ansuran) || 0}}</p>
        </div>
        <div class="border-b border-black flex flex-row text-caption pr-2 justify-end">
          <p class="font-semibold mr-4">Ansuran: </p>
          <p class="font-semibold">Rp. {{currencyFormat(ansuran)}}</p>
        </div>
        <div class="border-b border-black flex flex-row text-caption pr-2 justify-end">
          <p class="font-semibold mr-4">Sisa Tunggakan: </p>
          <p class="font-semibold">Rp. {{currencyFormat(dataIuran.sisaBayar - ansuran) || 0}}</p>
        </div>
      </div>
    </div>
...
export default Vue.extend({
  ...
  methods: {
    currencyFormat(iuran:Number) {
      return formatterCurrency(iuran)
    },
    async fetchProfil() {
